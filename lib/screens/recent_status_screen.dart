...
class RecentStatusScreen extends StatefulWidget {
  const RecentStatusScreen({super.key, required this.installDate});
  ...
  void fetchWeight() async {
    ...
    setState(() {
      if (data.isEmpty) {
        weightList = [
          WeightEntry(
              date: dateTimeToInt(widget.installDate),
              weight: context.watch<Setting>().weight!.toDouble())
        ];
      } else {
        weightList = data;
      }
          ...
          return CupertinoAlertDialog(
            ...
            actions: [
              ...
              CupertinoDialogAction(
                  ...
                  child: const Text('추가'),
                  onPressed: () async {
                    final hasData = await dbHelper
                        .hasWeightEntry(dateTimeToInt(DateTime.now()));

                    if (hasData) {
                      dbHelper.updateWeightEntry(
                        WeightEntry(
